...
BEGIN
  ...
  -- If plant_code is missing, prefer the leading numeric plant code from the location name
  -- e.g. "3802- BMW -MADURANTHAGAM" -> "3802". Otherwise use first 6 alnum chars.
  ...
  -- Final fallback only if plant_code, gate, and location name are unusable
  IF v_plant IS NULL OR length(v_plant) = 0 THEN
    v_plant := 'HO';
  END IF;
